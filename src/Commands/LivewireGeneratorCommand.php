...
abstract class LivewireGeneratorCommand extends Command {
  ...
  private function _getLayoutPath() {
    return $this->makeDirectory(resource_path("/views/components/layouts/app.blade.php"));
  }
  ...
  protected function buildLayout(): void {
    if (!(view()->exists($this->layout))) {
      ...
      if ($this->layout == 'layouts.app') {
        $this->files->copy($this->getStub('components/layouts/app', FALSE), $this->_getLayoutPath());
      }
      else {
  ...
  protected function modelReplacements() {
    ...
    [
      $relations,
      $properties,
    ] = (new ModelGenerator($this->table, $properties, $this->modelNamespace))->getEloquentRelations();
